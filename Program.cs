...
#pragma warning restore CS8604
    ...
    public class GameData {
        ...
        public static GameData LoadFromXml(string filePath) {
            ...
            XmlNode root = doc.SelectSingleNode("TextTrollSim");
            if(root == null) throw new Exception("Некорректный XML-файл");

            GameData gameData = new GameData();

            XmlNode gameNameNode = root.SelectSingleNode("GameName");
            if(gameNameNode != null) gameData.GameName = gameNameNode.InnerText;

            XmlNode authorNode = root.SelectSingleNode("Author");
            if(authorNode != null) gameData.Author = authorNode.InnerText;

            XmlNode trolleysNode = root.SelectSingleNode("Trolleybuses");
            if(trolleysNode != null) {
                foreach(XmlNode t in trolleysNode.SelectNodes("Trolleybus")) {
                    string model = t.Attributes["model"] != null ? t.Attributes["model"].Value : "Неизвестная модель";
                    string numbers = t.Attributes["numbers"] != null ? t.Attributes["numbers"].Value : "";
                    gameData.Trolleybuses.Add(new Trolleybus(model, numbers.Split(new char[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries)));
                }
            }

            XmlNode routesNode = root.SelectSingleNode("Routes");
            if(routesNode != null) {
                foreach(XmlNode r in routesNode.SelectNodes("Route")) {
                    string name = r.Attributes["name"] != null ? r.Attributes["name"].Value : "Без имени";
                    List<string> stops = new List<string>();
                    foreach(XmlNode s in r.SelectNodes("Stop")) stops.Add(s.InnerText);
